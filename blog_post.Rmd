...
Lastly, we analyzed whether the time span between the actual travel date and the submission of the review correlated with the rating: (1) we calculated the time difference in months, (2) we used the `geom_jitter` function to spread the overlapping dots, and (3) we introduced the `geom_crossbar` to display the mean rating of each group. The most interesting part of this plot is the `geom_jitter` function, which adds random noise to the plot to avoid overplotting. This visualization allows the reader to estimate the number of ratings in each category at first glance. The density of dots indicates that passengers generally had a favorable perception of their flight experiences and tend to review their flights shortly after their journey. The crossbar shows that the average rating increases with each month. Thus, as time passes, passengers tend to rate their experiences more positively.

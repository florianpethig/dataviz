...
As Business Data Analysts (BDAs) at zeroG, we try to create great visualizations and share them with our fellow BDAs during our monthly knowledge exchange meetings. For a long time, Jean-Philippe and I wanted to get our head around R visualizations. So we kicked off a little side project to analyze and visualize Lufthansa reviews from Tripadvisor.

### Data

We collected the data set for this article from Tripadvisor. It contains 1,385 English reviews of Lufthansa flights to or from Frankfurt. The reviews were published on Tripadvisor between January 2016 and January 2017. The data set contains the following variables.
...
For the next visualization, we chose a dot plot because it focuses on the rank ordering of items, which is especially suitable to display the best and worst performing flights. We used ggarrange to place the two plots next to each other. We only chose routes which had at least 5 reviews so that the mean rating is somewhat meaningful. The results indicate that only Cairo, Venice, and Amman have a rating below 3. Furthermore, we see that long-haul flights tend to attract more positive ratings than short- and medium-haul flights.
...
Next, we wanted to know whether any differences in the ratings across different booking classes existed. We used a mosaic plot because it divides the different booking classes into their relative proportions. We used the package "ggmosaic". The plot shows that the majority of travelers reviewed Economy class, followed by Business Class, Premium Economy, and First Class. Interestingly, travelers had a more positive perception of Lufthansa’s Premium Economy Class than of its Business Class.
...
Lastly, we analyzed whether the time span between the actual travel date and the submission of the review had an impact on the rating. First, we calculated the time difference in months. Second, we used the geom_jitter function to spread the overlapping dots. Third, we introduced the geom_crossbar to display the mean rating of each group. The results are very interesting. The density of dots indicates that passengers generally had a favorable perception of their flight experiences and tend to review their flights shortly after their journey. The crossbar shows that the average rating increases with each month. Thus, as time passes, passengers tend to rate their experiences more positively.
...
#Jitter plot
g <- ggplot(jitter, 
            aes(sort.int(date_dif), review_rating))
plot_jitter <- g + geom_crossbar(data=jitter_summary, 
                                 aes(x=date_dif, 
                                     ymin=mean, 
                                     ymax=mean, 
                                     y=mean, 
  ...
  labs(x = "Time Span Between Flight and Review",
       y = "Ratings",
       title = "Difference Between Flight Time and Review Time",
